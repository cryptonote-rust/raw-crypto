...
extern "C" {
  ...
  fn generate_key_derivation(public_key: *const u8, secret_key: *const u8, derivation: *mut u8) -> bool;
}
...
impl Key {
  pub fn generate_key_pair(public_key: &mut [u8; 32], secret_key: &mut [u8; 32]) {
    unsafe { generate_key_pair(public_key.as_mut_ptr(), secret_key.as_mut_ptr()) }
  }
  ...
  pub fn derive(public_key: &[u8; 32], secret_key: &[u8; 32]) -> [u8; 32] {
    let mut derived: [u8; 32] = [0;32];
    unsafe {
        generate_key_derivation(public_key.as_ptr(), secret_key.as_ptr(), derived.as_mut_ptr());
    }
    derived
  }
}
